...
def get_embeddings(config: RAGConfig | None = None):
    ...
    if custom.get("enabled") and custom.get("model") and custom.get("api_key"):
        ...
        model = custom["model"]
        kwargs = {"api_key": custom["api_key"], "model": model, "check_embedding_ctx_length": False}
        if base_url:
    ...
    kwargs = {"api_key": api_key, "model": model, "check_embedding_ctx_length": False}
    if base_url:
        kwargs["base_url"] = base_url
    return OpenAIEmbeddings(**kwargs)
